...
contains
  subroutine init_fruit(rank,test_unit_write)
    integer, intent(in), optional :: rank
    integer,intent(in),optional :: test_unit_write
    integer :: unit_write
    logical :: if_write

    if (present(test_unit_write)) then
       unit_write=test_unit_write
    else
       unit_write=stdout
    endif
    ...
    if (if_write) then
       write (unit_write,*)
       write (unit_write,*) "Test module initialized"
       write (unit_write,*)
       write (unit_write,*) "   . : successful assert,   F : failed assert "
       write (unit_write,*)
    endif
...
!$omp end critical (FRUIT_OMP_DEALLOCATE_MESSAGE_ARRAY)
  ...
  subroutine fruit_summary_xml_(path_to_outputs)
    character(1000),intent(in),optional :: path_to_outputs
    character(1000) :: path
    character(len = XML_LINE_LENGTH) :: whole_line
    character(len = 100) :: full_count
    character(len = 100) :: fail_count

    if (present(path_to_outputs)) then
       path=trim(path_to_outputs)//trim(xml_filename)
    else
       path=xml_filename
    end if
    ...
    open (XML_OPEN, file = path, action ="write", status = "replace")
    write(XML_OPEN, '("<?xml version=""1.0"" encoding=""UTF-8""?>")')
    write(XML_OPEN, '("<testsuites>")')
...
999 continue
  ...
  subroutine fruit_summary_(test_unit_write)
    integer,intent(in),optional :: test_unit_write
    integer :: unit_write
    integer :: i
    ...
    write (unit_write,*)
    write (unit_write,*)
    write (unit_write,*) '    Start of FRUIT summary: '
    write (unit_write,*)

    if (failed_assert_count > 0) then
       write (unit_write,*) 'Some tests failed!'
    else
       write (unit_write,*) 'SUCCESSFUL!'
    end if

    write (unit_write,*)
    if ( message_index > 1) then
       write (unit_write,*) '  -- Failed assertion messages:'

       do i = 1, message_index - 1
          write (unit_write,"(A)") '   '//trim(strip(message_array(i)))
       end do

       write (unit_write,*) '  -- end of failed assertion messages.'
       write (unit_write,*)
    else
       write (unit_write,*) '  No messages '
    end if
    ...
    write (unit_write, *) '  -- end of FRUIT summary'
  ...
  & test_unit_write)
    ...
    write (unit_write,*) 'Total asserts :   ', succ_assert + fail_assert
    write (unit_write,*) 'Successful    :   ', succ_assert
    write (unit_write,*) 'Failed        :   ', fail_assert
    write (unit_write,'("Successful rate:   ",f6.2,"%")')  real(succ_assert) * 100.0 / &
         real (succ_assert + fail_assert)
    write (unit_write, *)
    write (unit_write,*) 'Successful asserts / total asserts : [ ',&
         succ_assert, '/', succ_assert + fail_assert, ' ]'
    write (unit_write,*) 'Successful cases   / total cases   : [ ', succ_case, '/', &
         succ_case + fail_case, ' ]'
