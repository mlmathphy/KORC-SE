...
for iv=14:19
    for sl=1:nslice
        varr(:,:,sl,:,iv)=varr(:,:,1,:,14+(iv-14)+6*(sl-1));
    end
end
%rearranging J
for iv=20:25
    for sl=1:nslice
        varr(:,:,sl,:,iv)=varr(:,:,1,:,(14+6*nslice)+(iv-20)+6*(sl-1));
    end
end
%rearranging V
for iv=26:31
    for sl=1:nslice
        varr(:,:,sl,:,iv)=varr(:,:,1,:,(14+2*6*nslice)+(iv-26)+6*(sl-1));
    end
end
%rearranging P
for iv=32:33
    for sl=1:nslice
        varr(:,:,sl,:,iv)=varr(:,:,1,:,(14+3*6*nslice)+(iv-32)+2*(sl-1));
    end
end
%rearranging Pe
for iv=34:35
    for sl=1:nslice
        varr(:,:,sl,:,iv)=varr(:,:,1,:,(14+3*6*nslice+2*nslice)+(iv-34)+2*(sl-1));
    end
end
%rearranging ne
for iv=36:37
    for sl=1:nslice
        varr(:,:,sl,:,iv)=varr(:,:,1,:,(14+3*6*nslice+2*2*nslice)+(iv-36)+2*(sl-1));
    end
end
%rearranging conc
for iv=38:39
    for sl=1:nslice
        varr(:,:,sl,:,iv)=varr(:,:,1,:,(14+3*6*nslice+3*nslice)+(iv-38)+2*(sl-1));
    end
end
%rearranging Te
for iv=40:41
    for sl=1:nslice
        varr(:,:,sl,:,iv)=varr(:,:,1,:,(14+3*6*nslice+4*nslice)+(iv-40)+2*(sl-1));
    end
end
%rearranging Ti
for iv=42:43
    for sl=1:nslice
        varr(:,:,sl,:,iv)=varr(:,:,1,:,(14+3*6*nslice+5*nslice)+(iv-42)*2*(sl-1));
    end
end
