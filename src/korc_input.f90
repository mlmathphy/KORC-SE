module korc_input
  ...
  REAL(rp) :: zmax =  1.65
  REAL(rp) :: zmin = -1.65
  CHARACTER(75) :: outputs_list = '{X,Y,V,B,E,g,eta,flagCon,flagCol,PSIp,ne,Flag}'
    ! List of outputs
  ...
  REAL(rp) :: minor_radius = 0.6
    ! Minor radius in meters. ITER: 1.5 DIII-D: 0.5
  REAL(rp) :: major_radius = 1.6
...
CONTAINS
  ...
  subroutine read_namelist(params,infile,echo_in,outdir)
    ...
    NAMELIST /plasma_species/ ppp,q,m,Eno,etao,Eo_lims,etao_lims,runaway, &
         ...
         pinit
    NAMELIST /analytical_fields_params/ Bo,perturb,l_mn,sigma_mn,eps_mn,minor_radius,major_radius,&
